$(function() {
  ...
  var tileTypes = [
    ...
    wallType = new TileType('wall', 15,6, 120,120,120, "Wall", {wallSolids: 0xff, image:$("#walltiles").attr("url")}), // encoding: bit 0 is noon, goes clockwise
    wallBottomLeftType = new TileType('wallBottomLeft', 15,7, 128,112,64, "Wall BL", {wallSolids: 0xb4, image:$("#walltiles").attr("url")}),
    wallTopLeftType = new TileType('wallTopLeft', 15,9, 64,128,80, "Wall TL", {wallSolids: 0xd2, image:$("#walltiles").attr("url")}),
    wallTopRightType = new TileType('wallTopRight', 15,10, 64,80,128, "Wall TR", {wallSolids: 0x4b, image:$("#walltiles").attr("url")}),
    wallBottomRightType = new TileType('wallBottomRight', 15,8, 128,64,112, "Wall BR", {wallSolids: 0x2d, image:$("#walltiles").attr("url")}),
...
//      console.log('mouse left ', $(this).data('x'), $(this).data('y'));
  ...
  $("#saveToJJ").click( function() {
    $(this).text("Saving...");
  $.post('/editorsave',{layout: getPngBase64(),logic: JSON.stringify(makeLogic())}
    ).done(function (data) {
      if(data.message == true) {
        ...
        alert("Error saving to Juke Juice");
      }
